...
class HtmltotextValidationException implements Exception {
  final List<String> errors;

  HtmltotextValidationException(this.errors);

  @override
  String toString() => 'HtmltotextValidationException: ${errors.join("; ")}';
}
...
/// HTML to Text API client.
///
/// For more information, visit: https://htmltotext.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [html] (required) - The HTML to convert to text
class HtmltotextClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'html': ValidationRule(type: 'string', required: true),
  };

  HtmltotextClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/htmltotext',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HtmltotextValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HtmltotextValidationException(errors);
    }
  }

  /// Execute a request to the HTML to Text API.
  ///
  /// Parameters are validated before sending the request.
  Future<HtmltotextResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HtmltotextException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HtmltotextResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HtmltotextException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HtmltotextException('Resource not found');
      } else {
        throw HtmltotextException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HtmltotextException) rethrow;
      throw HtmltotextException('Request failed: $e');
    }
  }
...
/// Exception thrown by the HTML to Text API client.
class HtmltotextException implements Exception {
  final String message;

  HtmltotextException(this.message);

  @override
  String toString() => 'HtmltotextException: $message';
}
